...
if(isset($_POST['search1']) && !empty($_POST['search1'])){
    $tab = $c->listShiftbynurseid($_POST['search1']);
}elseif(isset($_POST['submit']) && !empty($_POST['submit'])){
    $tab = $c->listArchivedShifts();
}
else
$tab = $c->listShift();
?>
...
<br>
    ...
    <button type="submit" name ="submit" value="submit">Archived Shifts</button>
    </form>
    <button type="button" onclick="window.location.href='listShift.php'">Unarchived Shifts</button>
    </center>

    <form autocomplete="off" align="center" action="" method="POST">
        <b><label for="search_nav">Write Nurse Name</label></b>
        <div class="autocomplete">
            <input type="text" id="myInput" name="search1" placeholder="Name...">
        </div>
    ...
    <table>
        ...
        ?>
            <tr>
                ...
                <?php if($shift['archived'] == 0){ ?>
                <td><button type="button" onclick="window.location.href='archiveshift.php?shift_id=<?php echo $shift['shift_id']; ?>'">Archive</button></td>
                <?php }else{ ?>
                <td><button type="button" onclick="window.location.href='unarchivedshift.php?shift_id=<?php echo $shift['shift_id']; ?>'">Unarchive</button></td>
                <?php } ?>
                <td><button type="button" onclick="window.location.href='deleteshift.php?shift_id=<?php echo $shift['shift_id']; ?>'">Delete</button></td>
            </tr>
        <?php
        }
        ?>
        ...
        <script>
        //extract nurse_name from database
        <?php $names = $c->listnursename(); ?>
        nurse_names = <?php echo json_encode($names); ?>;
        ...
        autocomplete(document.getElementById("myInput"), nurse_names);
    </script>
